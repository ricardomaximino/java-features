...
public class Address {
    ...
    public Address(long id) {
        this.id = id;
        this.country = coutries[(int) Math.round(Math.random() * (coutries.length -1))];
        this.town = twons[(int) Math.round(Math.random() * (twons.length -1))];
        this.number = String.valueOf(Math.round(Math.random() * 10000));
    }
